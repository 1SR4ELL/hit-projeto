/**
 * seedTest.js — Dados fictícios para testes de conversas
 * Rodar: node src/utils/seedTest.js
 */
require('dotenv').config()
const { encrypt, hash } = require('./crypto')
const prisma = require('./prismaClient')

const tenantId_DEMO = null // preenchido dinamicamente
...
const conversas = [
  ...
  },
  {
    ...
    msgs: [
      ...
      { role: 'human_agent', content: 'Obrigado pela informação, Roberto. Vamos apurar. Você pode nos enviar fotos ou documentos pelo e-mail juridico@campanha.com? Sua identidade será preservada.' },
    ],
  },
  {
    waId: 'test_voter_005',
    ...
    msgs: [
      ...
      { role: 'assistant', content: 'Falou sim! O programa de saúde mental inclui: CAPS (Centros de Atenção Psicossocial) em todos os distritos, psicólogos nas escolas municipais, grupo de apoio para dependência química e linha de apoio 24h para crise. É uma das prioridades do Dr. João! 💙' },
    ],
  },
  {
    waId: 'test_voter_006',
  ...
  },
  {
    ...
    msgs: [
      ...
      { role: 'human_agent', content: 'Olá Paula! Que ótimo. Nossa agenda está sendo organizada. Pode me passar seu e-mail e telefone que entro em contato amanhã pela manhã?' },
    ],
  },
]

async function main() {
  console.log('🌱 Inserindo dados de teste de conversas...\n')
  ...
  console.log(`✅ Tenant encontrado: ${tenant.nomeCandidato}\n`)
  ...
  for (const conv of conversas) {
    ...
    // Cria sessão com histórico cifrado
    ...
    console.log(`  ✓ ${conv.nome} (${conv.bairro}) — ${conv.msgs.length} msgs${conv.escalonada ? ' 🔴 escalada' : ''}`)
  }

  console.log(`\n🎉 ${conversas.length} conversas de teste criadas!`)
  console.log('   Acesse o dashboard → Conversas para visualizar.')
}
