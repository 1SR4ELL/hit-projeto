...
router.get('/', async (req, res) => {
  try {
    ...
    const [ags, total] = await Promise.all([
      prisma.agendamento.findMany({ where, include: { eleitor: true }, orderBy: { agendadoPara: 'desc' }, skip: (Number(page)-1)*Number(limit), take: Number(limit) }),
      prisma.agendamento.count({ where })
    ])
    res.json({ agendamentos: ags, total })
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!templateNome || !agendadoPara) return res.status(400).json({ error: 'templateNome e agendadoPara obrigatorios' })
