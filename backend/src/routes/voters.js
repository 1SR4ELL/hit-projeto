...
router.get('/', async (req, res) => {
  try {
    const { search, optIn, interesse, page=1, limit=20 } = req.query
    ...
    const [eleitores, total] = await Promise.all([
      prisma.eleitor.findMany({ where, orderBy: { createdAt: 'desc' }, skip: (Number(page)-1)*Number(limit), take: Number(limit) }),
      prisma.eleitor.count({ where })
    ])
    res.json({ eleitores: eleitores.map(e => ({ ...e, whatsappNumberEnc: undefined })), total, page: Number(page), pages: Math.ceil(total/Number(limit)) })
...
router.get('/:id', async (req, res) => {
  try {
    const e = await prisma.eleitor.findFirst({ where: { id: req.params.id, tenantId: req.tenantId }, include: { sessoes: { orderBy: { createdAt: 'desc' }, take: 5 } } })
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    await prisma.eleitor.updateMany({ where: { id: req.params.id, tenantId: req.tenantId }, data: { nomePreferido, municipio, bairro, interessePrincipal, zonaEleitoral, secaoEleitoral } })
    res.json({ ok: true })
  } catch (err) { res.status(500).json({ error: err.message }) }
})
